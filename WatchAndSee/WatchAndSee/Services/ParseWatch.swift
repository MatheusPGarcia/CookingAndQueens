...
class ParseWatch: NSObject {
    ...
    func sendToWatch(steps: [Step]) -> [String: Any?] {
    ...
    func decodeInWatch(_ dataArray: [String: Any?]) -> [Step] {
        ...
        for stp in singleData {
            ...
            guard let order = stp["Order"] as? Int else { return steps }
            let time = stp["Tempo"] as? Int

            let newStep = Step(text: text, time: time, order: order)
